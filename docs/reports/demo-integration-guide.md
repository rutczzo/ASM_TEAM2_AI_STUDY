...
API 키 없이도 기본 규칙 기반 분석과 `bm25` 검색으로 동작합니다. Solar 기반 확인 답변 판정,
약점 보정과 후보 재순위화를 함께 보려면 루트 `.env`에 `UPSTAGE_API_KEY`를 설정하세요.
Vector/Hybrid 검색은 추가로 Qdrant 설정이 필요합니다.
...
PowerShell에서는 가상환경 활성화 명령으로 `.\.venv\Scripts\Activate.ps1`을 사용합니다.
...
**③ 확인 질문 왕복 (세션)**
②에서 선택지 클릭 → "답변하고 계속"
→ 누적 입력과 병합되어 추천 카드 또는 다음 확인 질문으로 진행
(서버가 session_id로 누적 입력과 질문 횟수를 기억하며, 최대 2회 후 분석 진행).

> 입력 화면의 **예시 칩**을 누르면 텍스트가 자동으로 채워집니다.

**④ Solar 의도 재순위화 (API 키 설정 시)**
입력 예시:
```
인프라를 잘하고 싶지만 일반 DevOps보다 로그 관측과 장애 대응 역량을 배우고 싶습니다.
```
→ 약점의 구체 검색 힌트와 후보 프로필을 Solar가 비교해 범용 인프라 후보와 SRE 후보의
상대 순위를 보정합니다.
...
## 알려진 제약
- 멘토 데이터는 20명의 합성 프로필입니다. 실제 멘토 데이터와 장기 세션 저장은 범위 밖입니다.
- 카드 적합도는 절대 평가가 아니라 현재 요청과 후보 안에서의 상대적 적합도입니다.

---

## 무엇을 보는 데모인가 (검증 포인트)
- 노드1(입력 파싱·충분성) → 노드2(검색) → 노드3(적합도) → 노드4(카드)까지 **한 번의 요청으로 관통**.
- 입력 부족 시 **확인 질문 분기**, 답변 후 **누적 세션 병합 재요청**, 최대 2회 종료 조건.
- API 키 설정 시 Solar 기반 약점 보정과 후보 의도 재순위화, 실패 시 규칙 기반 폴백.
