...
## 🔧 알려진 이슈 — 추후 고도화 과제 (데모 마감엔 영향 없음)
**실서버 모드에서 로딩 화면(S-02, StepProgress)이 보이지 않음.**
- 원인: 로딩 연출용 인위적 지연이 `useRecommend.ts`의 **목 모드 분기(`if (USE_MOCK)`)에만** 있음
  (`await sleep(1800ms)`). 실서버는 응답이 ~10ms로 매우 빨라 로딩 화면이 깜빡이고 바로
  결과로 넘어가, 단계 점등 애니메이션이 사람 눈에 안 보임. (컴포넌트는 삭제 안 됨 — 정상 존재)
- 영향: 기능엔 문제 없음(입력→결과 정상). 분석 진행 연출 UX만 안 보임.
- 해결안(미적용): 실서버 모드에도 최소 로딩 표시 시간 부여 —
  `await Promise.all([callServer(req), sleep(MIN_LOADING)])` 형태. **프론트 담당 협의 후 적용 예정.**
- 결정(2026-06-07): 데모 공유 우선, 본 이슈는 **고도화 과제로 보류.**
