...
- 확인 질문(`CLARIFY`)은 한 화면에 **1개씩** 표시한다. 백엔드는 최대 2회의
  `need_clarification`을 반환할 수 있으므로 매 응답의 `status`를 그대로 따라 다시 S-03을 표시한다.
...
interface Mentor {
  ...
  domain: string[];
  keywords: string[];
  score: number;               // 현재 요청 안에서의 상대 적합도 0~100
...
3. **확인 질문은 한 화면에 항상 1개.** 여러 질문을 한 번에 노출하지 않으며, 후속
   `need_clarification` 응답은 같은 S-03 화면에서 이어서 처리한다.
